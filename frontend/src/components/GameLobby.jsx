...
          event.id === "upcoming" ? event.startTime : event.endTime
        );
...
            seconds,
            isEnded: false,
            isUpcoming: event.id === "upcoming", // New flag to distinguish
          };
        } else {
          return {
            isEnded: true,
...
              const isUpcoming = event.id === "upcoming";
              return (
                <div
                  key={event.id}
                  className={`bg-black/20 rounded-xl p-5 my-4 border border-slate-700 transition-all transform hover:scale-[1.02] ${
                    isUpcoming
                      ? "hover:border-purple-500 hover:shadow-lg hover:shadow-purple-500/30"
                      : "hover:border-green-500 hover:shadow-lg hover:shadow-green-500/30"
                  } relative`}
                >
                  <div className="flex justify-between items-start">
                    <h2
                      className={`text-xl font-bold ${
                        isUpcoming ? "text-purple-400" : "text-green-400"
                      }`}
                    >
                      {event.name}
                    </h2>
...
                            Event {isUpcoming ? "Starts Soon!" : "Ended 🏁"}
                          </span>
                        ) : (
                          <span className="whitespace-nowrap">
                            {isUpcoming ? "Starts in" : "Time left"}: {time.days > 0 && `${time.days}d `}
...
                  <div className="flex items-center gap-3">
                    <button
                      className={`flex-grow font-bold py-3 px-4 rounded-lg transition-all shadow-lg ${
                        isUpcoming
                          ? "bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white cursor-not-allowed opacity-70"
...
                      disabled={isUpcoming}
                    >
                      {isUpcoming ? "Upcoming" : "Join Event"}
                    </button>
                    <button
                      className="p-2 bg-slate-700/80 hover:bg-slate-600 rounded-lg transition-colors"
...
                    >
                      <img
                        src={isUpcoming ? MyLeaderboardIcon_B : MyLeaderboardIcon_G}
                        alt="Leaderboard"
                        className="h-8 w-8"
                      />
                    </button>
